...
func init() {
	onet.SimulationRegister("SwUpRandClient", NewRandClientSimulation)
}
...
func NewRandClientSimulation(config string) (onet.Simulation, error) {
...
// rounds
func (e *randClientSimulation) Run(config *onet.SimulationConfig) error {
		...
		if dr.Time.Sub(now) >= updateFrequency || i == len(drs)-1 {
			...
			for k, v := range latest {
				...
				if updates != nil {
					...
					rel := releases[k]
					monitor.RecordSingleMeasure("download_binary", float64(rel.BinariesSize))
