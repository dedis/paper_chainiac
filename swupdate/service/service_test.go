...
func insertChain(service *Service, r *onet.Roster, c *packageChain) *SwupChain {
	cpr, err := service.CreatePackage(nil,
...
func checkChain(s *Service, r *onet.Roster, name string, last *SwupChain) error {
...
// func verifyExistence(t *testing.T, hosts []*onet.Host, sc *SwupChain,
...
func initGlobals(nbrKeys int) {
	...
	createBlock := func(name, version string) *packageBlock {
		...
		p1, err := network.Marshal(policy1)
		log.ErrFatal(err)
		var sigs1 []string
		for _, k := range keys {
			s1, err := k.Sign(p1)
